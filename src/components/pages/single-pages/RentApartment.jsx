...
import { rentApartmentHeaderData } from '../rent/rentData';
import { emptyListData } from '../rent/rentData';
...
const RentApartment = () => {
    ...
    const filteredData =(value)=> {
      if (value === 'verified-apartments') {
        const data = rentApartmentData.filter((item) => item.property_type === 'verified');
        return data
      }
      if (value === 'premium-apartments') {
        const data = rentApartmentData.filter((item) => item.property_type === 'premium');
        return data
      }
      if (value === 'bedspaces') {
        const data = rentApartmentData.filter((item) => item.property_type === 'bedspace');
        return data
      }
      if (value === 'shared-rent-apartments') {
        const data = rentApartmentData.filter((item) => item.property_type === 'shared');
        return data
      }
    }
    const data = filteredData(id)

    const filterHeaderData =(value)=> {
      const data = rentApartmentHeaderData.filter((item) => item.name === value);
      return data
    }
    const headerData = filterHeaderData(id);
  ...
  return (
    <div className='rent-apartment'>
        <PageHeader pageHeader={headerData[0]} overlay={true}/>
            ...
            <div className="right-section">
              <div className="right-section-content">
                ...
                : currentItems.map((item) => (
                    <RentCard key={item.id} rentCardData={item} small={true}/>
                  ))}
              </div>
              {currentItems.length === 0 ? '' :
